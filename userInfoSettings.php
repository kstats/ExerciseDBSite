			...
			else
{
		...
		<ul class="menu" style="text-align:right"> 
			...
			<li><a href = userInfoSettings.php style="color: black">Settings</a></li>
			<li><a href = searchUser.php style="color: black">Search Users</a></li>
		</ul>

		<div class ="testing">
	<blockquote><font face="CENTURY GOTHIC" size="7" color="#9999FF">Update User Profile</font></blockquote></div><br><br><br><br><br><br>
  ...
  return $theValue;
	...
	while($row = mysql_fetch_array($retval, MYSQL_ASSOC))
	{
		echo'<div class="quote">
			...
			<form name="test" method="POST" > 
			<label>&nbsp &nbsp &nbsp &nbsp Height:</label> 
			<input type="text" name="Height" value='.$row['Height'] .'><br><br>
			<form name="test" method="POST" > 
			<label>&nbsp &nbsp &nbsp &nbsp Weight:</label> 
			<input type="text" name="Weight" value='.$row['Weight'] .'><br><br>
				
<form name="test" method="POST" > 
				<label>&nbsp &nbsp &nbsp &nbsp Gender: </label>	
				
			<select name="Gender">
			<option selected="selected">'.$row['Gender'] .'</option>
...
<form name="test" method="POST" > 
				...
				<input type="text" name="Zip" value='.$row['Zip Code'] .' > <br>
				<br>
<form name="test" method="POST" > 
...
$dbhost = 'localhost';
 ...
 {
	 $test = "SELECT * FROM mydb.user WHERE `username` LIKE '$_SESSION[MM_Username]'";
	 mysql_select_db('USER');
...
$pass =					GetSQLValueString(isset($_POST['password1']) ? $_POST['password1'] : null, "text");
   $gender =                		GetSQLValueString($_POST['Gender'], "text");
  $address =                     	GetSQLValueString(isset($_POST['Address']) ? $_POST['Address'] : null, "text");
   ...
   $height =                    	GetSQLValueString(isset($_POST['Height']) ? $_POST['Height'] : null, "int");
    $weight =                    GetSQLValueString(isset($_POST['Weight']) ? $_POST['Weight'] : null, "int");
    $bmi =                    GetSQLValueString(isset($_POST['BMI']) ? $_POST['BMI'] : null, "double");
 ...
 $order = "UPDATE mydb.user SET email = $email, 
 `Gender` = $gender, Address = $address, City = $city, password = $pass,
 State = $state, `Zip Code` = $zip, Height = $height,
 Weight = $weight, BMI = $bmi, `First Name` = $first, `Last Name` = $last WHERE username LIKE '$_SESSION[MM_Username]'";
mysql_select_db('USER');
